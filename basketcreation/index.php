...
session_start(); 
/*
 * include http_request function to handle GET, POST, PUT, DELETE request methodes
 */
 ...
 */
define('SHOP_ID', 669437); // replace with your classic shopId

define('PLATFORM', 'net'); // net or com - depends on where your account is registered

define('LOCALE', 'de_DE'); // e.g. en_EU, fr_FR, en_GB, us_US

define('API_KEY', '');     // add your api key here

define('API_SECRET', '');  // add your api secret
/*
...
if (isset($_POST['size']) and isset($_POST['appearance']) and isset($_POST['quantity'])) {
  ...
  /*
   * make a redirect to the basket location
   */
 ...
 */
$articles = simplexml_load_file('http://api.spreadshirt.' . PLATFORM . '/api/v1/shops/' . SHOP_ID . '/articles?locale=' . LOCALE . '&fullData=true&limit=20');
...
foreach ($articles->article as $article) {
  ...
  $output .= '<img src="' . (string)$article->resources->resource->attributes('xlink', true) . '" alt="' . $article->name . '" class="preview" />';
  /*
